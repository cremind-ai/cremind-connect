...
2. opens the system browser to Google's consent screen
   ...
   `scope=openid email <the resource's own scopes>`, `access_type=offline`);
...
- **Drive** `changes.watch(...)` over the same channel mechanism →
  `/ingress/google/drive`. Under `drive.file` the feed reports only the files the
  user granted.

Gmail has **no** event plane: the shared client requests no restricted scope, so it
cannot call `users.watch()` (nor read a mailbox). Gmail is execution-only
(`gmail.send`); mailbox reading and live mail events run over IMAP inside the app,
never through this relay.

`routingKey = base32(sha256("google:" + lower(trim(email)))[:16])` is computed by
the app and embedded in the watch channel id; the relay parses it back out, routes
to Durable Object `AccountHub(routingKey)`, and broadcasts a nudge
`{type:"resync", source}` to every connected app. Each app then performs its own
incremental sync (`events.list` / `changes.list`) locally with its own token.
...
| Receive push | relay | no (verifies the provider's signal) | — |
| Fetch changes | local app | yes (its own) | no |
| Refresh token | local app | yes (its own) | no |
| Route events | relay | no (key parsed from the channel id) | never stored |
...
- Calendar/Drive: the app embeds the key in the channel id (`cm-<key>-<nonce>`, ≤64
  chars); the relay parses it from `X-Goog-Channel-ID`. No stored channel→account
  mapping.
- One hub multiplexes all resources for an account (calendar, drive, jira);
...
sync. The relay is a real-time accelerator, not a durable queue. (If a sync token
has expired after a long offline period, the app falls back to a bounded recent
resync — an app-side concern.)

## Security model / threat analysis

- **Forged Calendar/Drive webhook** (unsigned by Google) → low impact: it can at most
...
|---|---|---|
| account email | ID-token `email` at `/subscribe` | hashed to routingKey, then discarded; never stored/logged |
| Calendar/Drive channel headers | webhook | routingKey parsed; not stored |
| OAuth access/refresh tokens | — | **never seen** |

Logging uses a field allowlist (`src/lib/log.ts`): only the derived `routingKey`
(already a hash) and counts are logged — never email, cursors, tokens, or JWTs.
...
Resources with no event plane (Gmail, Contacts/Sheets/Docs) simply register no
ingress adapter — they keep a discovery entry for their scopes and their skills are
execution-only. File-level changes to Sheets/Docs surface via the Drive resource's
changes.watch feed. Drive itself is push: it reuses the Calendar `web_hook` channel
pipeline (`/ingress/google/drive`, shared channel-id parser and HMAC knob), so it
was a new ingress adapter, not new infrastructure.
...
- The shared client requests **only sensitive scopes, no restricted ones**
  (`gmail.send`, `calendar.events`, `drive.file`, `spreadsheets`, `documents`).
  Public launch therefore needs Google verification but **no** third-party security
  assessment (CASA), which attaches to restricted scopes alone. Development uses
  the consent screen in "Testing" mode (≤100 test users, no verification). Adding
  a restricted scope would change this — see SETUP-GCP.md.
